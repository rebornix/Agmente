...
- ACP session persistence must never save unresolved placeholder IDs as durable sessions. When a placeholder resolves, migrate any locally cached or persisted transcript state onto the real session ID so Copilot-style reloads can reopen the same conversation instead of duplicate or blank rows.
- ACP placeholder-session failures must be terminal for that draft: if `session/new` fails, clear the placeholder session selection before any prompt path can fall through to `session/prompt` with the placeholder ID.
...
- Codex turns inferred only from `thread/resume` are resume-derived state, not confirmed live turns: keep them locally resettable in the composer, and only restore real `turn/interrupt` behavior after a successful loaded-thread attach/read or a live turn notification (`turn/started`, `item/started`, `item/completed`).
